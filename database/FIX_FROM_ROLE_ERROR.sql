-- ============================================
-- SOLUCIÓN ERROR CASTEOS EN HISTORIAL (FROM_ROLE)
-- ============================================

-- El error indica que la tabla 'participant_changes' TAMBIÉN usa el tipo especial 'selection_role'
-- en las columnas 'from_role' y 'to_role'.
-- Debemos añadir el casteo ::selection_role también en el INSERT.
...
BEGIN
  ...
  -- 3. Actualizar selección (CON CASTEO ::selection_role)
  ...
  -- 4. Registrar historial (CON CASTEO ::selection_role TAMBIÉN AQUÍ)
  -- Nota: Eliminamos matchday porque no existe.
  INSERT INTO participant_changes (
    ...
    from_team_id, 
    to_team_id, 
    from_role, -- Tipo selection_role
    to_role,   -- Tipo selection_role
    league, 
    executed_at
  ) VALUES (
    p_season_id, 
    ...
    p_team_out_id, 
    p_team_in_id,
    v_role_out::selection_role, -- <--- CORRECCIÓN AQUÍ
    v_role_in::selection_role,  -- <--- CORRECCIÓN AQUÍ
    v_league_out, 
    NOW()
  );
